...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference("Target.count") do
      post targets_url, params: { target: { name: @target.name, program_url: @target.program_url } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { name: @target.name, program_url: @target.program_url } }
